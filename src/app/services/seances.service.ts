  ...
  constructor(private msService: moviesServices,
    ...
    this.http.get('http://localhost:8080/seansList').subscribe((data) => {
      ...
      for (let i = 0; i < this.jsonvar.length; i++) {
        ...
        this.http.get('http://localhost:8080/movies/' + this.jsonvar[i].id).subscribe(data => {
          const movie = new MovieModel(data[0].id,
            data[0].nazwa,
            data[0].img,
            data[0].czasTrwania,
            data[0].ocena);
          this.http.get('http://localhost:8080/roomsById/' + data[0].id).subscribe(data => {
            const room = new Room(data[0].id, data[0].miejsca);
            this._seancesList.push(new SeanceModel(movie, this.jsonvar[i].data, this.jsonvar[i].czas, room));
          });
